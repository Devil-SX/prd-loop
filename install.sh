...
PY_MODULE_DIR="$PROJECT_DIR/src/prd_loop"
...
export PRD_LOOP_WORKDIR="\$(pwd)"
cd "$PROJECT_DIR"
exec uv run python3 "$PY_MODULE_DIR/spec_to_prd.py" "\$@"
EOF
...
export PRD_LOOP_WORKDIR="\$(pwd)"
cd "$PROJECT_DIR"
exec uv run python3 "$PY_MODULE_DIR/impl_prd.py" "\$@"
EOF
...
main() {
    ...
    echo ""
    log "SUCCESS" "PRD Loop installed successfully!"
    echo ""
